...
> 目的：按”需求点”聚合 PRD、demo、web、后端、算法 的落点，解决需求在仓库中分散的问题。第一版目标：demo验证→web发布→后端就绪→算法集成。
...
| 文档 | 回答什么问题 | 不负责什么 |
|------|--------------|------------|
| `docs/prd/*.md` | 这个需求为什么做、用户要什么、验收口径是什么 | 不负责写四层文件落点 |
| `docs/features/*.md` | 这个需求在当前仓库里落到哪里、demo→web→后端→算法怎么映射、当前是否对齐 | 不替代 PRD，不记录逐日开发流水 |
| `docs/progress/*.md` | 当前做到哪一步、阻塞在哪里、这轮改了什么 | 不替代需求定义，不承担跨层总映射 |
...
1. 一个需求会同时影响 `PRD + 至少一层实现（demo/web/后端/算法）`
2. 一个需求会同时影响 `demo / web / 后端` 中的两层及以上
3. 一个需求依赖 `Supabase + AI Server` 能力，并且需要和 demo/web 实现一起对齐
4. 一个需求需要长期跟踪”各层是否对齐”，而不是只记录一次性开发动作
...
- 当前仓库已开始补齐“按需求点聚合”的中间层
- 本目录用于持续扩展这层结构
- 当前已落地的映射文档：
  - `REQ-010-wish-create-and-clarify-flow.md`
  - `REQ-012-theme-switching-and-role-companion.md`
